...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		ArrayList<GridPos> childPos = new ArrayList<GridPos>();
		SearchNode curr = frontier.removeFirst();
		
		while (!p.isGoalState(curr.getState())) {
			childPos = p.getReachableStatesFrom(curr.getState());
			
			for (GridPos child : childPos) {
				SearchNode childNode = new SearchNode(child, curr);
