...
public class QLearningController extends Controller {
	...
	final static int NUM_ACTIONS = 4;//7; /* The takeAction function must be changed if this is modified */
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				double qValue = oldQValue + alpha(Ntable.get(prev_stateaction)) * (previous_reward + GAMMA_DISCOUNT_FACTOR * (getMaxActionQValue(new_state)) - oldQValue);
