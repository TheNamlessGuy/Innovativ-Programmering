...
/**
 * Removes x if it exists, and rebalances the tree if needed.
 */
template<typename Comparable>
void
AVL_Tree_Node<Comparable>::
remove(const Comparable& x, Node_Pointer& t)
{
  if (t == nullptr) 
  {
    throw AVL_Tree_error("Va fan tror du håller på med din lilla skit");
  }
  ...
  if (x < t->element) 
  {
    ...
    //cout << t->element << ": " << (node_height(t->right) - node_height(t->left)) << endl;
    if (max(node_height(t->right), node_height(t->left)) - min(node_height(t->right), node_height(t->left)) == 2)
    {
      //cout << "Debug: x=" << x << "  t->right->element=" << t->right->element << endl;
      //cout << "x > t->right->element" << endl;
  ...
  else if (t->element < x) 
  {
    ...
    //cout << t->element << ": " << (node_height(t->left) - node_height(t->right)) << endl;
    if (max(node_height(t->right), node_height(t->left)) - min(node_height(t->right), node_height(t->left)) == 2)
    {
      //cout << "Debug: x=" << x << "  t->left->element=" << t->left->element << endl;
