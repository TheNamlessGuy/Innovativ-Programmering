...
class ArithmeticConstraint
  ...
  def new_value(connector)
    if a.has_value? and b.has_value? and (not out.has_value?) then 
      ...
      # B and OUT have values
      val = out.value.send(inverse_op, b.value)
      logger.debug("#{self} : #{a} updated")
      a.assign(val, self)
    elsif a.has_value? and (not b.has_value?) and out.has_value? then
      # A and OUT have values
...
def celsius2fahrenheit
  ...
  celsmult = Connector.new("(C x 9/5)")
  mult = ConstantConnector.new("9/5", 9/5.0)
  add = ConstantConnector.new("32", 32)
  
  Multiplier.new(celsius, mult, celsmult)
  Adder.new(celsmult, add, fahrenheit)
