...
class ConstraintParser < Parser
  ...
  def get_connectors(conn_a,op,conn_b)
    if (!conn_a.is_a?(Connector))
      if (conn_a.a.name.include?(conn_a.inverse_op.to_s))
        conn_a = conn_a.a
      elsif (conn_a.b.name.include?(conn_a.inverse_op.to_s))
        conn_a = conn_a.b
      else
        conn_a = conn_a.out
      end
    ...
    if (!conn_b.is_a?(Connector))
      if (conn_b.a.name.include?(conn_b.inverse_op.to_s))
        conn_b = conn_b.a
      elsif (conn_b.b.name.include?(conn_b.inverse_op.to_s))
        conn_b = conn_b.b
      else
        conn_b = conn_b.out
      end
